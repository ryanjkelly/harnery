						...
						<div>
							<div className="flex flex-wrap items-center gap-2">
								...
								<ObserverBadge
									...
									stale={report.service.stale}
								/>
								{snapshot ? (
									<SupportBadge state={snapshot.support.state} />
								) : null}
							</div>
							<p className="mt-2 max-w-3xl text-sm text-muted-foreground">
								See local CPU and memory pressure by process tree. Expensive
								unmatched processes stay visible.
							</p>
						</div>
							...
							{snapshot ? (
								<>
									<div>
										Captured{" "}
										<FormattedDateTime
											iso={snapshot.sampled_at}
											kind="datetime"
										/>
									</div>
									<div className="mt-1">
										{formatAge((report.freshness_ms ?? 0) / 1_000)} ago ·{" "}
										{snapshot.namespace}
									</div>
								</>
							) : (
						...
						<Card className="border-amber-500/30 bg-amber-500/5">
							<CardHeader>
								<CardTitle className="text-base">
									Waiting for the resource observer
								</CardTitle>
							</CardHeader>
							<CardContent className="text-sm text-muted-foreground">
								The dashboard starts the local supervisor automatically. Service
						...
						<>
							...
							>
								...
								<MetricCard
									...
									tone={
										snapshot.unattributed_process_count > 0
											? "warning"
											: "success"
									}
								/>
							...
							<section aria-labelledby="supervisor-heading" className="mb-6">
								<div className="mb-3 flex flex-wrap items-end justify-between gap-3">
									<div>
										...
										<p className="mt-1 text-sm text-muted-foreground">
											Service health, hook activity, recent pressure, and
								...
								<div className="grid gap-3 xl:grid-cols-[1.1fr_1fr]">
									<Card>
										...
										<CardContent>
											<div className="grid gap-2 sm:grid-cols-2">
												{(supervisor.snapshot?.services ?? []).map(
													(service) => (
														<ServiceHealthRow
															key={service.id}
															service={service}
									...
									<Card>
										...
										<CardContent>
											<MiniHistoryChart
												points={supervisor.history?.points ?? []}
											/>
										</CardContent>
									</Card>
								</div>
							...
							{supervisor.hookHealth ? (
								<section
									aria-labelledby="completed-hooks-heading"
									className="mb-6"
								>
									<div className="mb-3 flex flex-wrap items-end justify-between gap-3">
										<div>
											...
											<p className="mt-1 text-sm text-muted-foreground">
												Recent terminal receipts. Memory is the hook process RSS
												at completion, not a process-tree peak.
											</p>
										</div>
										<div className="flex flex-wrap items-center gap-2 text-xs">
											<Badge
												variant={
													supervisor.hookHealth?.capability.state ===
													"supported"
														? "secondary"
														: "warning"
												}
											...
											<span className="text-muted-foreground">
												{supervisor.hookHealth?.summary.invocation_count ?? 0}{" "}
												receipts ·{" "}
												{supervisor.hookHealth?.summary.degraded_count ?? 0}{" "}
												degraded ·{" "}
												{supervisor.hookHealth?.summary.faulted_count ?? 0}{" "}
												faulted
											</span>
										</div>
									</div>
									<p className="mb-2 text-xs text-muted-foreground sm:hidden">
										Swipe sideways to see every hook metric.
									</p>
									<section
										className="overflow-x-auto rounded-xl border border-border/60 bg-card"
										aria-label="Completed hook health table"
									>
													...
													<tr>
														<td colSpan={10} className="px-3 py-5 text-center">
															<span className="text-sm text-muted-foreground">
																No completed hook receipts are visible in the
																bounded log window.
															</span>
														</td>
													</tr>
												...
												<div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
													<div className="min-w-0">
														...
														<div className="mt-1 text-xs text-muted-foreground">
															{group.process_count} process
															{group.process_count === 1 ? "" : "es"}
														</div>
													</div>
													<Badge
														variant={
												...
												<div className="mt-4 grid grid-cols-2 gap-3 text-sm tabular-nums">
													<div>
														...
														<div className="font-semibold">
															{formatPercent(group.cpu_percent)}
														</div>
													</div>
													<div>
														<div className="text-xs text-muted-foreground">
								...
								<div className="mb-3 flex flex-wrap items-end justify-between gap-3">
									<div>
										...
										<p className="text-sm text-muted-foreground">
											Sorted by current CPU, then resident memory. Every shown
											row remains searchable.
										</p>
									</div>
									<div className="text-xs text-muted-foreground">
										Supervisor resource overhead{" "}
										{formatMs(snapshot.collector_cpu_ms)} CPU per sample
									</div>
								</div>
								<div className="overflow-x-auto rounded-xl border border-border/60 bg-card">
												...
												<div className="flex items-start justify-between gap-3">
													<div className="min-w-0">
														...
														<div className="mt-1 font-mono text-[11px] text-muted-foreground">
															PID {row.pid} · {formatAge(row.age_seconds)} ·{" "}
															{row.name}
														</div>
													</div>
...
function CompletedHookRow({
	...
	return (
		<tr className="align-top hover:bg-muted/30" data-hook-health-key={hook.key}>
			...
			<Td align="right">
				<Badge variant={problems > 0 ? "warning" : "secondary"}>
					{problems}
				</Badge>
			</Td>
			<Td>
				...
				>
					{hook.owner_ids.length > 0 ? (
						...
						<li className="text-xs text-muted-foreground">Unknown</li>
					)}
				</ul>
			</Td>
			<Td>
...
function MetricCard({
	...
	const toneClass = {
		success:
			"border-emerald-500/25 bg-emerald-500/5 text-emerald-600 dark:text-emerald-300",
		warning:
		...
		<div className="flex items-center justify-between gap-3 rounded-lg border border-border/60 px-3 py-2">
			<div className="min-w-0">
				...
				<div className="mt-0.5 text-[10px] text-muted-foreground">
					{service.pid
						? `PID ${service.pid}`
						: (service.reason ?? "No live process")}
				</div>
			</div>
...
function MiniHistoryChart({
	...
	const memory = chartPoints(
		points.map((point) => point.machine.memory_percent),
	);
			...
			<CardContent className="p-4">
				...
				{finding.attribution || finding.workload_context ? (
					<div className="mt-3 flex flex-wrap gap-x-3 gap-y-1 text-xs text-foreground/75">
						{finding.attribution ? (
							<span>{findingOwnerLabel(finding)}</span>
						) : null}
						{finding.workload_context ? (
							<span>
								{finding.workload_context.relationship.replaceAll("-", " ")}
							</span>
						) : null}
					</div>
				) : null}
...
function SupportBadge({ state }: { state: string }) {
	return (
		<Badge variant={state === "supported" ? "secondary" : "warning"}>
			{state}
		</Badge>
	);
}
