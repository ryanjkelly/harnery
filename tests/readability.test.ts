...
const md = (html: string, selector = "div"): string =>
  htmlToMarkdown(`<div>${html}</div>`, { selector }).output;

describe("htmlToMarkdown: base conversion", () => {
  test("headings render as ATX style", () => {
    expect(md("<h1>One</h1>")).toContain("# One");
    expect(md("<h2>Two</h2>")).toContain("## Two");
    expect(md("<h3>Three</h3>")).toContain("### Three");
  });

  test("bullet lists use '-' marker", () => {
    expect(md("<ul><li>one</li><li>two</li></ul>")).toMatch(/-\s+one[\s\S]*-\s+two/);
  });

  test("code blocks render as fenced", () => {
    const out = md("<pre><code>console.log(1)</code></pre>");
    expect(out).toMatch(/```[\s\S]*console\.log\(1\)[\s\S]*```/);
  });

  test("links use [text](href) form", () => {
    expect(md(`<a href="https://example.com">click</a>`)).toContain(
      "[click](https://example.com)",
    );
  });

  test("inline emphasis is preserved", () => {
    expect(md("<em>x</em>")).toContain("_x_");
    expect(md("<strong>x</strong>")).toContain("**x**");
  });

  test("scripts/styles/iframes/noscript are stripped", () => {
    const out = md(
...
describe("htmlToMarkdown: GFM tables", () => {
  test("table with thead+tbody becomes a markdown table", () => {
    const out = md(
  ...
  test("table with implicit thead (first row is <th>) still converts", () => {
    const out = md(
  ...
  test("empty cells preserve row shape", () => {
    const out = md(
  ...
  test("three-column table renders all columns", () => {
    const out = md(
  ...
  test("regression: cells do NOT flatten into a vertical text dump", () => {
...
describe("htmlToMarkdown: GFM strikethrough", () => {
  test("<del> renders as ~~text~~", () => {
    expect(md("<del>gone</del>")).toContain("~~gone~~");
  });

  test("<s> renders as ~~text~~", () => {
    expect(md("<s>gone</s>")).toContain("~~gone~~");
  });

  test("deprecated <strike> renders as ~~text~~", () => {
    expect(md("<strike>gone</strike>")).toContain("~~gone~~");
  });
});

describe("htmlToMarkdown: selector mode", () => {
  test("extracts content matching selector and discards the rest", () => {
    const html = `<div><h1>Skip</h1><article><p>Keep</p></article></div>`;
    const r = htmlToMarkdown(html, { selector: "article" });
    expect(r.output).toContain("Keep");
    expect(r.output).not.toContain("Skip");
  });

  test("title is null when bypassing Readability", () => {
    const r = htmlToMarkdown("<div><p>x</p></div>", { selector: "div" });
    expect(r.title).toBeNull();
  });

  test("throws when selector matches nothing", () => {
    expect(() => htmlToMarkdown("<p>x</p>", { selector: ".nope" })).toThrow(/Selector/);
  });
});

describe("htmlToMarkdown: raw mode", () => {
  test("returns cleaned HTML instead of markdown", () => {
    const r = htmlToMarkdown("<div><h1>Hi</h1></div>", { selector: "div", raw: true });
...
describe("htmlToMarkdown: input validation", () => {
  test("throws on empty input", () => {
    expect(() => htmlToMarkdown("")).toThrow(/Empty/);
  });
...
describe("htmlToMarkdown: Readability integration", () => {
  test("extracts article body + title, skips nav/footer chrome", () => {
...
<footer>footer noise that should be excluded</footer>
  ...
  test("throws if Readability finds no article", () => {
    // Tiny HTML with no recognizable article body.
    expect(() => htmlToMarkdown("<html><body></body></html>")).toThrow();
  });
});

describe("htmlToMarkdown: pre-Turndown DOM preprocessing", () => {
  test("<br> inside <td> becomes a space, not a newline", () => {
  ...
  test("<br> inside <th> becomes a space", () => {
    const out = md(
  ...
  test("<br> OUTSIDE table cells still produces a line break", () => {
    // Make sure we only flattened <br>s inside <td>/<th>, not all <br>s.
    const out = md("<p>line one<br>line two</p>");
    // Turndown emits "  \n" for a paragraph <br>.
    expect(out).toMatch(/line one\s*\n\s*line two/);
  });

  test("<pre> with nested element tokens collapses to textContent", () => {
  ...
  test("<pre><code> (canonical code block) is left intact", () => {
  ...
  test("plain-text <pre> (no element children) passes through unchanged", () => {
    const out = md("<pre>raw text only</pre>");
    expect(out).toContain("raw text only");
  });
});

describe("htmlToMarkdown: truncation", () => {
  test("output is capped at maxChars with a truncation marker", () => {
    const big = "<p>" + "x".repeat(5000) + "</p>";
    const r = htmlToMarkdown(big, { selector: "p", maxChars: 100 });
  ...
  test("maxChars: 0 disables truncation", () => {
    const big = "<p>" + "x".repeat(5000) + "</p>";
    const r = htmlToMarkdown(big, { selector: "p", maxChars: 0 });
