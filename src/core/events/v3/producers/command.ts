...
export type CommandSignalV3 = "command.started" | "command.output_observed" | "command.completed";

export interface CommandProducerContextV3 {
...
export interface CommandObservationV3 {
...
export function normalizeCommandEventV3(
  signal: CommandSignalV3,
  observation: CommandObservationV3,
  context: CommandProducerContextV3,
): EventV3 | null {
  ...
  switch (signal) {
    case "command.started": {
      ...
      return buildEventV3("command.started", {
        ...common,
        payload: {
          ...
          exact_command: fingerprintV3(context.fingerprintContext, "exact-command", argv),
          intent_kind: safeToken(observation.intent_kind ?? "unknown", "unknown"),
          intent_length: Buffer.byteLength(intent, "utf8"),
          ...(intent
            ? { intent_fingerprint: fingerprintV3(context.fingerprintContext, "intent", intent) }
            : {}),
    ...
    case "command.output_observed": {
      ...
      return buildEventV3("command.output_observed", {
        ...common,
        payload: {
          ...
          ...(observation.output === undefined
            ? {}
            : {
                content_fingerprint: fingerprintV3(
                  context.fingerprintContext,
    ...
    case "command.completed": {
      if (!context.terminal_span) return null;
      const outcome = observation.outcome ?? outcomeFromExit(observation.exit_code);
      return buildEventV3("command.completed", {
        ...common,
        payload: {
          outcome,
          ...
          ...(observation.signal
            ...
            : {}),
          duration_ms: context.terminal_span.duration_ms,
...
): NonNullable<CommandObservationV3["outcome"]> {
