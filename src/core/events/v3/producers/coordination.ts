import type { AuthorityMutationV3 } from "../authority-outbox.ts";
import { buildEventV3 } from "../builder.ts";
import { type FingerprintContextV3, fingerprintV3, normalizeNativeIdV3 } from "../canonical.ts";
import type { EventV3, SpanSummaryV3 } from "../contract.ts";
import { eventIdV3 } from "../ids.ts";
import { describePathTargetV3 } from "../targets.ts";

export type CoordinationAuthoritySignalV3 =
  | "task-changed"
  | "lifecycle-changed"
  | "claim-changed"
  | "identity-attested"
...
export interface CoordinationProducerContextV3 {
  coordRoot: string;
...
interface CoordinationObservationBaseV3 {
  native_observation_id: string;
}

export interface TaskChangedObservationV3 extends CoordinationObservationBaseV3 {
  state: "set" | "cleared";
  prior_state?: string;
  task?: string;
}

export interface LifecycleChangedObservationV3 extends CoordinationObservationBaseV3 {
  state: "active" | "blocked" | "done";
  prior_state?: string;
  reason_code?: string;
}

export interface ClaimChangedObservationV3 extends CoordinationObservationBaseV3 {
  operation: "acquired" | "released";
  target: unknown;
  access: "read" | "write";
}

export interface IdentityAttestedObservationV3 extends CoordinationObservationBaseV3 {
  identity_id: string;
  method: string;
}

export interface DecisionStateChangedObservationV3 extends CoordinationObservationBaseV3 {
  decision_id: string;
  prior_state?: string;
...
export interface WaitStartedObservationV3 extends CoordinationObservationBaseV3 {
  wait_id: string;
  kind: "permission" | "approval" | "decision" | "operator_input" | "dependency" | "scheduled";
  wake_at?: string;
}

export interface WaitEndedObservationV3 extends CoordinationObservationBaseV3 {
  wait_id: string;
  outcome:
    | "succeeded"
    | "failed"
...
export interface CoordinationObservationBySignalV3 {
  "task-changed": TaskChangedObservationV3;
  "lifecycle-changed": LifecycleChangedObservationV3;
  "claim-changed": ClaimChangedObservationV3;
  "identity-attested": IdentityAttestedObservationV3;
  "decision-state-changed": DecisionStateChangedObservationV3;
  "wait-started": WaitStartedObservationV3;
  "wait-ended": WaitEndedObservationV3;
}
 ...
 */
export function normalizeCoordinationAuthorityV3<S extends CoordinationAuthoritySignalV3>(
  signal: S,
  observation: CoordinationObservationBySignalV3[S],
  context: CoordinationProducerContextV3,
): NormalizedCoordinationAuthorityV3 {
  ...
  const sourceRecordId = normalizeNativeIdV3(
    context.fingerprintContext,
    "agent-coord.authority-source",
    observation.native_observation_id,
  );
  const common = {
    event_id: context.event_id ?? eventIdV3(),
    producer: {
  ...
  switch (signal) {
    case "task-changed": {
      const input = observation as TaskChangedObservationV3;
      ...
      const taskFingerprint =
        input.state === "set"
          ? fingerprintV3(context.fingerprintContext, "coord.task", input.task, "root")
          : undefined;
      return {
        event: buildEventV3("coord.task_changed", {
          ...common,
          payload: {
    ...
    case "lifecycle-changed": {
      ...
      return {
        event: buildEventV3("coord.lifecycle_changed", {
          ...common,
          payload: {
            actor_instance_id: context.actor_instance_id,
            subject_instance_id: context.subject_instance_id,
    ...
    case "claim-changed": {
      const input = observation as ClaimChangedObservationV3;
      const target =
        typeof input.target === "string"
          ? describePathTargetV3({
              ...
              extractorVersion: "agent-coord-claim-v3",
            })
          : {
              ...
              extractor_version: "agent-coord-claim-v3",
            };
      return {
        event: buildEventV3("coord.claim_changed", {
          ...common,
          payload: {
            actor_instance_id: context.actor_instance_id,
            subject_instance_id: context.subject_instance_id,
    ...
    case "identity-attested": {
      const input = observation as IdentityAttestedObservationV3;
      const identityId = safeToken(input.identity_id);
      return {
        event: buildEventV3("coord.identity_attested", {
          ...common,
          payload: {
            actor_instance_id: context.actor_instance_id,
            subject_instance_id: context.subject_instance_id,
    ...
    case "decision-state-changed": {
      const input = observation as DecisionStateChangedObservationV3;
    ...
    case "wait-started": {
      const input = observation as WaitStartedObservationV3;
      const waitId = safeToken(input.wait_id);
      return {
        event: buildEventV3("wait.started", {
          ...common,
          payload: {
            wait_id: waitId,
            kind:
              input.kind === "operator_input"
                ? "needs_input"
    ...
    case "wait-ended": {
      if (!context.terminal_span) throw new Error("wait terminal requires span evidence");
      const input = observation as WaitEndedObservationV3;
      const waitId = safeToken(input.wait_id);
      return {
        event: buildEventV3("wait.ended", {
          ...common,
          payload: {
            wait_id: waitId,
            outcome: input.outcome,
            resolution_reference: context.transaction_id,
            span: context.terminal_span,
          },
        }) as EventV3,
