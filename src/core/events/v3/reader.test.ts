...
import { type LedgerFrameV3, readLedgerFramesV3, readLedgerFramesV3Since } from "./reader.ts";
...
function fixture(eventType: string, sequence: number): Fixture {
  ...
  const event = object(sample(branch));
  const id = `00000000-0000-7000-8000-${String(sequence).padStart(12, "0")}`;
  event.event_id = `evt_${id}`;
  object(event.contract).schema_digest = EVENT_V3_SCHEMA_DIGEST;
  object(event.producer).sequence = sequence;
  object(event.links).caused_by = [];
  ...
  if (eventType === "ledger.activated") {
    object(event.payload).eligible_after_event_id = event.event_id;
  }
  return event;
}

function frame(event: Fixture, segmentOrdinal: number, byteOffset: number): LedgerFrameV3 {
  return {
