...
export const EVENT_V3_LIVE_RELATIVE_ROOT = ".harnery/live/v3" as const;

export interface LiveDisplayInputV3 {
...
export interface LiveDisplayRowV3 {
...
export interface LiveDisplayJanitorResultV3 {
  scanned: number;
  removed: number;
  retained: number;
}
 ...
 */
export function writeLiveDisplayV3(
  coordRoot: string,
  input: LiveDisplayInputV3,
  now: () => Date = () => new Date(),
): LiveDisplayRowV3 {
  ...
  if (!Number.isSafeInteger(ttlMs) || ttlMs <= 0 || ttlMs > HARD_MAX_TTL_MS) {
    throw new Error("V3 live-display TTL must be between 1 ms and 1 hour");
  }
  if (input.executable !== undefined && !EXECUTABLE_PATTERN.test(input.executable)) {
    throw new Error("V3 live-display executable is invalid");
  }
  const writtenAt = now();
  const intentDisplay = safeIntentDisplayV3(input.intent_display);
  const targetLabels = safeTargetLabels(input.target_labels);
  const row: LiveDisplayRowV3 = {
    format: "harnery-event-v3-live-display",
    format_version: 1,
 ...
 */
export function tryWriteLiveDisplayV3(
  coordRoot: string,
  input: LiveDisplayInputV3,
  now: () => Date = () => new Date(),
): LiveDisplayRowV3 | undefined {
  try {
    return writeLiveDisplayV3(coordRoot, input, now);
  } catch {
    return undefined;
  }
}
...
export function readLiveDisplayV3(
  coordRoot: string,
  generationId: string,
  now: () => Date = () => new Date(),
): LiveDisplayRowV3[] {
  if (!GENERATION_PATTERN.test(generationId))
    throw new Error("V3 live-display generation ID is invalid");
...
export function janitorLiveDisplayV3(
  coordRoot: string,
  now: () => Date = () => new Date(),
): LiveDisplayJanitorResultV3 {
  const root = liveRoot(coordRoot);
...
export function safeIntentDisplayV3(value: string | undefined): string | undefined {
  ...
  const safe = values
    ...
    .filter(
      (value) =>
        ...
        safeIntentDisplayV3(value) !== undefined,
    );
  return [...new Set(safe)];
}

function validateLiveRow(value: unknown): LiveDisplayRowV3 {
  ...
  if (Object.keys(row).some((key) => !allowed.has(key)))
    throw new Error("invalid live-display row");
  if (
    row.format !== "harnery-event-v3-live-display" ||
  ...
  if (
    ...
    safeIntentDisplayV3(row.intent_display) !== row.intent_display
  ) {
    throw new Error("invalid live-display row");
  }
  if (
  ...
  return row as unknown as LiveDisplayRowV3;
}
...
function assertLiveIds(generationId: string, eventId: string): void {
  if (!GENERATION_PATTERN.test(generationId) || !EVENT_PATTERN.test(eventId)) {
    throw new Error("V3 live-display identity is invalid");
  }
}
