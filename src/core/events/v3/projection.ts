...
export type SafetyProjectionDiagnosticCodeV3 =
  | "ledger_incomplete"
...
export interface SafetyProjectionDiagnosticV3 {
  code: SafetyProjectionDiagnosticCodeV3;
  event_id?: string;
  generation_id?: string;
...
export interface WaitProjectionV3 {
  wait_id: string;
  kind: string;
...
export interface GenerationSafetyStateV3 {
  generation_id: string;
  instance_id: string;
  session_id: string;
  started_event_id: string;
  ...
  waits: Record<string, WaitProjectionV3>;
  progress_event_ids: string[];
  last_context_event_id?: string;
  resume_count: number;
}

export interface ClaimProjectionV3 {
  key: string;
  subject_instance_id: string;
...
export interface DelegationProjectionV3 {
...
export interface SafetyProjectionV3 {
  ...
  generations: Record<string, GenerationSafetyStateV3>;
  current_generation_by_instance: Record<string, string>;
  claims: Record<string, ClaimProjectionV3>;
  delegations: Record<string, DelegationProjectionV3>;
...
/**
 * Deterministically fold only contract-validated V3 rows. Physical ledger
 ...
 */
export function reduceSafetyProjectionV3(read: ReadLedgerV3Result): SafetyProjectionV3 {
  const projection: SafetyProjectionV3 = {
...
function applyEvent(projection: SafetyProjectionV3, positioned: PositionedEventV3): void {
...
function applySessionStart(
  ...
  positioned: PositionedEventV3,
  event: Extract<EventV3, { event_type: "session.started" }>,
): void {
  const scope = event.scope as GenerationScopeViewV3;
...
function applyClaim(
  projection: SafetyProjectionV3,
  event: Extract<EventV3, { event_type: "coord.claim_changed" }>,
): void {
...
function applyProvisionalTermination(
  projection: SafetyProjectionV3,
  event: Extract<EventV3, { event_type: "session.termination_observed" }>,
): void {
  const generationId = projection.current_generation_by_instance[event.payload.subject_instance_id];
  const state = generationId ? projection.generations[generationId] : undefined;
...
function applySweepObservation(
  projection: SafetyProjectionV3,
  event: Extract<EventV3, { event_type: "lifecycle.sweep_observed" }>,
): void {
  const generationId = projection.current_generation_by_instance[event.payload.subject_instance_id];
  const state = generationId ? projection.generations[generationId] : undefined;
  if (state?.phase !== "live") return;
...
function applyAgentCompleted(
  projection: SafetyProjectionV3,
  event: Extract<EventV3, { event_type: "agent.completed" }>,
): void {
...
function generationIdOf(event: EventV3): string | undefined {
  return "generation_id" in event.scope ? event.scope.generation_id : undefined;
}

function touch(state: GenerationSafetyStateV3, positioned: PositionedEventV3): void {
...
function addDiagnostic(
  projection: SafetyProjectionV3,
  diagnostic: Omit<SafetyProjectionDiagnosticV3, "authority_blocking">,
): void {
...
function authorityAttributionVerified(event: EventV3, subjectInstanceId?: string): boolean {
