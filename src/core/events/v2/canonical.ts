...
export const EVENT_V2_CANONICALIZER = "harnery-jcs-nfc-v1" as const;

export type JsonPrimitive = null | boolean | number | string;
export type JsonValue = JsonPrimitive | JsonValue[] | { [key: string]: JsonValue };

/**
 * Canonical JSON used for V2 contract digests, event bytes, and fingerprints.
 *
 * It follows RFC 8785's ECMAScript number and string serialization, adds the
 * V2 NFC rule for keys and values, and rejects values that JSON would silently
...
export function sha256V2(value: string | Uint8Array): `sha256:${string}` {
  return `sha256:${createHash("sha256").update(value).digest("hex")}`;
}

export interface FingerprintV2 {
  algorithm: "hmac-sha256";
  canonicalizer: typeof EVENT_V2_CANONICALIZER;
  key_epoch: `pep_${string}`;
  scope: "generation" | "root";
  digest: `sha256:${string}`;
}

export interface FingerprintContextV2 {
  epochId: `pep_${string}`;
  epochKey: Uint8Array;
  rootId: `root_${string}`;
...
export function fingerprintV2(
  ...
  const rootKey = Buffer.from(
    hkdfSync(
      ...
      Buffer.from(`harnery:v2:root\0${context.rootId}`, "utf8"),
      32,
    ),
  );
  ...
  if (scope === "generation") {
    ...
    comparisonKey = Buffer.from(
      hkdfSync(
        ...
        Buffer.from("harnery:v2:generation", "utf8"),
        32,
      ),
    );
  }
  const digest = createHmac("sha256", comparisonKey)
    .update(`harnery:v2:${domain}\0`, "utf8")
    .update(canonicalJsonV2(value), "utf8")
    .digest("hex");
  return {
    algorithm: "hmac-sha256",
    canonicalizer: EVENT_V2_CANONICALIZER,
...
export function normalizeNativeIdV2(
  context: FingerprintContextV2,
  namespace: string,
  nativeId: string,
): `hid_${string}` {
  ...
  const digest = createHmac("sha256", rootKey)
    .update(`harnery:v2:native-id\0${namespace}\0`, "utf8")
