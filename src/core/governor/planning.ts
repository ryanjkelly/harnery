...
function plannerReviewScript(
  ...
  return [
    ...
    "        'Check that consequential contracts and program-shape decisions are explicit in work objectives or acceptance criteria rather than deferred to implementation.',",
    "        'Treat a cross-layer plan that defers every observable integration until the final item as blocking unless earlier layer-only items have independent evidence or are necessary prerequisites.',",
    ...
    "      'Apply proposal_root_rule exactly; never require proposal.root to equal active_root.',",
    "      'Keep consequential contracts and program-shape decisions explicit, and prefer independently observable end-to-end slices over layer-only batches.',",
    "      'Goal context: ' + JSON.stringify(goalContext),",
