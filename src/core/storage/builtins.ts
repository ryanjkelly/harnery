...
function durableHistoryFamilies(): HarneryStorageFamily[] {
  ...
  return [
    ...histories,
    family({
      id: "agent-mailbox-history",
      owner: "agent mailbox service",
      storage_class: "durable-object-history",
      roots: (context) => [subtree(context, ".harnery/mailbox")],
      format: "jsonl",
      durability: "crash-safe",
      writer_model: "multi-process",
      policy: ownerHistoryPolicy("agent-mailbox-history-v1"),
      consumers: ["agent mailbox service", "prompt delivery", "storage inventory"],
      provider: filesystemProvider("agent-mailbox-provider", "agent mailbox lifecycle"),
    }),
    family({
      id: "workflow-approval-history",
      owner: "workflow approval service",
      storage_class: "durable-object-history",
      roots: (context) => [subtree(context, ".harnery/approvals")],
      format: "json",
      durability: "crash-safe",
      writer_model: "object-owned",
      policy: ownerHistoryPolicy("workflow-approval-history-v1"),
      consumers: ["workflow approval service", "governor service", "storage inventory"],
      provider: filesystemProvider("workflow-approval-provider", "workflow approval lifecycle"),
    }),
    family({
      id: "agent-name-history",
      owner: "agent identity service",
      storage_class: "durable-object-history",
      roots: (context) => [exact(context, ".harnery/.name-history", "file")],
      format: "jsonl",
      durability: "crash-safe",
      writer_model: "multi-process",
      policy: ownerHistoryPolicy("agent-name-history-v1"),
      consumers: ["agent identity service", "session rendering", "storage inventory"],
      provider: filesystemProvider("agent-name-history-provider", "agent identity lifecycle"),
    }),
    family({
