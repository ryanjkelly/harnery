...
`harn web up` and `harn web start` record request timing and event-loop delays
to `.harnery/logs/web-performance.jsonl`. The recorder runs below Next.js, so
it covers pages, route handlers, React Server Component refreshes, and requests
...
overlapped them. A large request duration with little event-loop delay points
toward asynchronous I/O or framework compilation. A large event-loop delay
points toward synchronous filesystem work, CPU work, garbage collection, or a
descheduled process. It is evidence about overlap, not proof that one listed
request caused the delay.
