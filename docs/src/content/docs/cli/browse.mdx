...
Driven Chromium keeps the selected desktop UA, client-hint headers and
`navigator.platform` consistent. This includes the first document of a popup.
Headed sessions use a real 1536 by 864 window by default; the content area follows
the window and the screen dimensions come from the display. Pass `--viewport`
for an emulated QA size. Headless default remains 1280 by 800.

Plain login only changes the UA string: with nothing attached, its client hints
and navigator platform remain native. Fonts, speech voices and debugger-related
signals are outside the identity override. See [ADR 0186](/decisions/0186-consistent-browser-identity/).
...
| `--plain` | With `--login`: open the URL in the installed Google Chrome with nothing attached, on the same persistent profile. For sign-in flows whose verification step detects DevTools automation itself. Sign in, close the window (or create the close file); later `browse` and `browse-session` runs inherit the cookies. Not compatible with `--control-file`. |
...
| `--browser-channel <name>` | Browser to launch: `chrome`, `chrome-beta`, `msedge`, or `chromium`. Headed launches default to the installed Google Chrome with automation signals hidden; `chromium` forces the bundled build. `HARNERY_BROWSER_CHANNEL` sets the machine-wide default. |
| `--user-agent <ua\|auto\|native>` | User agent to present. Default: a normal Windows (macOS on a Mac) Chrome string at the launched Chrome's major, stored at `~/.cache/harnery/user-agent.json` and reused by `browse`, `browse-session`, and `fetch`; never Linux, never HeadlessChrome. A string rewrites the store, `auto` refreshes it, `native` uses the browser's own. `HARNERY_BROWSER_UA` overrides per environment. |
...
Harnery never presents as desktop Linux. Web application firewalls commonly deny an `X11; Linux` user agent outright (Sunbelt Network's Cloudflare rule answered 200 the moment the browser said Windows), and headless builds announce `HeadlessChrome`. Every page load uses a normal Chrome user agent for the host the operator sits at, at the real Chrome major, persisted machine-locally so all surfaces reuse the one that works.

Some sign-in verification steps detect the DevTools protocol itself, not just launch flags; X fails with "Something went wrong" at its verification gate under any Playwright-driven window. For those, run `harn browse --login --plain <url>`: Harnery spawns the installed Google Chrome directly on the shared profile with nothing attached, you sign in as in any browser, and once the window closes every later `browse` or `browse-session` run opens that profile already signed in.

A headed window (`--login` or `--headed`) launches the operator's installed Google Chrome when one exists at a standard path, drops Playwright's `--enable-automation` default, and disables Blink's AutomationControlled feature. Playwright's bundled browser is "Chrome for Testing" and announces itself through its brand list and `navigator.webdriver`; sign-in flows on sites that screen for automation silently refuse clicks a person makes in that window. Headless runs keep the bundled Chromium so page QA renders stay stable. Pass `--browser-channel chromium` to force the bundled build in a headed window.

On WSL, some Windows/GPU/WSLg combinations never present a headed window's GPU-composited surface, so the window shows in the taskbar but paints blank (the page still runs). `browse` used to add `--disable-gpu` to every headed launch under WSL for that case. It no longer does: software WebGL reports the SwiftShader renderer, which sign-in flows treat as a bot signal, and current WSLg exposes the host GPU through D3D12. If a headed window paints blank on your machine, set `HARNERY_BROWSER_WSL_DISABLE_GPU=1`; add other flags with `--browser-arg` or `HARNERY_BROWSER_ARGS`. Headed sessions also run with the Chromium sandbox on, so the "unsupported command-line flag: --no-sandbox" banner is gone.
...
Harnery reads pages at human pace by default. Consecutive loads of the same registrable site (`www.example.com` and `example.com` count as one site) wait a random 3 to 9 seconds, the way a person reads one page before opening the next. The reservation ledger at `~/.cache/harnery/pace.json` is shared by every process, so a shell loop of separate `harn fetch` calls is paced the same as one long `harn browse` session, and concurrent agents queue behind each other instead of firing together. Different sites never wait on each other. Loopback and private-network addresses, single-label hosts, and the `.localhost`, `.local`, `.test`, `.internal`, `.example`, and `.invalid` suffixes are exempt, so local development and page QA keep machine speed. A wait is logged to stderr as `paced 5.2s before example.com (human pace; --no-pace or HARNERY_PACE=off skips)`.

| Control | Effect |
| --- | --- |
| `--no-pace` | Skip the gap for this run. |
| `HARNERY_PACE=off` | Disable the gate machine-wide. |
| `HARNERY_PACE_MIN_MS`, `HARNERY_PACE_MAX_MS` | Change the gap window. Defaults `3000` and `9000`. |
| `HARNERY_PACE_EXEMPT` | Comma-separated host suffixes that never wait, for sites you operate. |
| `HARNERY_PACE_LEDGER` | Ledger path override. |

[`harn browse-ai`](/cli/browse-ai/), [`harn fetch`](/cli/fetch/), [`harn webview`](/cli/webview/), and the [`harn browse-session`](/cli/browse-session/) navigation verbs share the same gate and controls. Embedding hosts can pace their own page loaders through `harnery/lib/pace`. Decision record: [ADR 0184](/decisions/0184-read-pages-at-human-pace/).
...
`--review-pack` is the capture half of the capture-then-judge split behind
[`harn review-pack`](/cli/review-pack/) and [`harn qa-run`](/cli/qa-run/).
...
| Flag | Description |
| --- | --- |
| `--review-pack <dir>` | Write this render into the pack at `<dir>`. Pair with `--no-screenshot`: the pack holds the full-page image. |
| `--review-pack-context <id>` | Context id inside the pack (default `<viewport>-<theme>-<state>` from `--viewport`, `--qa-theme`, `--qa-state`). |
| `--review-pack-scope <selector>` | Also tile one screenshot per matching element (repeatable). Scope tiles follow the full-page bands in the tile order. |
| `--review-pack-plan` | Emit all native tile candidates and stable capture metadata without capturing the pack. Known gate failures still emit a plan. |
| `--review-pack-allocation <file>` | Capture exactly the selected IDs from a validated context allocation. Source or geometry changes require a new plan. Page edges remain required. |
| `--review-pack-reservation <file>` | Use the preliminary allocation only as a tile-count reservation. Repeat supplied gates and allocate on the current page; require unchanged full source and complete native coverage. One source-change retry is allowed. Cannot combine with allocation, expansion or inline critique. |
| `--review-pack-hit-rect <x,y,w,h>` | A document-space rectangle a deterministic gate flagged (repeatable, at most 50). A band that contains one is captured as a `hit band` even when it lies below the tile cap, so the reviewer gets the tile that shows the defect. `qa-run` passes each context's gate rectangles automatically. |
| `--review-pack-expand <tile-id>` | Instead of capturing a context, re-render one existing tile of `--review-pack-context` at `--device-scale-factor` and write it beside the original as `tiles/<tile>@<dpr>x.png`. Existing tiles are untouched. This is the child [`harn review-pack expand`](/cli/review-pack/) runs. |
| `--device-scale-factor <n>` | Device scale factor for the browser context (1 to 4; default 1). Every screenshot renders at that multiple of its CSS pixels. |
