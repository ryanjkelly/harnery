...
On WSL, a headed Chromium window composites its GPU-backed surface over an RDP stream to Windows; on many Windows/GPU/WSLg combinations that surface never presents, so the window shows in the taskbar but paints blank (the page still runs — JS, navigation, and DOM all work). `browse` therefore auto-adds `--disable-gpu` to headed launches under WSL, which forces the software compositor and restores on-screen paint. Opt out with `HARNERY_BROWSER_NO_WSL_DEFAULTS=1`; add alternative flags with `--browser-arg` or `HARNERY_BROWSER_ARGS`.
:::
