...
4. **Critique.** Only when every gate and interaction passed and the manifest requires a visual pass. One context at a time (the critique provider owns tile concurrency), with `--check-critique-fail`, `--qa-reuse` when a baseline exists, and, in signoff mode, `--qa-snapshot` riding in the same invocation. Unless the job permits metered critique, children run with `HARNERY_CRITIQUE_HEADLESS_ONLY=1`; an exhausted headless list becomes an `incomplete` blocker naming the policy, never a metered call. Full-page critique reviews at most `policy.critique_max_tiles` bands per context (default 24, the `browse --check-critique-max-tiles` default) and drops the rest of a taller page; each critique row records `coverage` (page height, reviewed height, bands total and reviewed, `capped`). In signoff mode a capped context is a `critique` blocker and the run is `incomplete`; in review mode the row keeps its outcome and carries the flag.
