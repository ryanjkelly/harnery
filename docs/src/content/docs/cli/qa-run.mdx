...
4. **Capture.** A visual manifest requires a capture plan from every gate render, including renders with known gate failures. The runner allocates one tile budget across those plans before starting capture. It reserves the top and bottom bands of every context, prioritizes bands containing gate findings, then distributes remaining slots by uncovered height. Scope tiles use the same budget. Each child receives the exact selected candidate IDs and verifies source, context, and geometry before capturing. A missing or changed plan makes the run incomplete. A failed gate still stays failed.
