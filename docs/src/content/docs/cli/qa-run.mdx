...
4. **Critique.** Only when every gate and interaction passed and the manifest requires a visual pass. One context at a time (the critique provider owns tile concurrency; full-page critique reviews at most 24 bands per context, the `browse --check-critique-max-tiles` default, and a taller page is truncated at that cap), with `--check-critique-fail`, `--qa-reuse` when a baseline exists, and, in signoff mode, `--qa-snapshot` riding in the same invocation. Unless the job permits metered critique, children run with `HARNERY_CRITIQUE_HEADLESS_ONLY=1`; an exhausted headless list becomes an `incomplete` blocker naming the policy, never a metered call.
