...
**Gate-hit bands.** When [`harn qa-run`](/cli/qa-run/) captures a context, it passes every rectangle its deterministic gates recorded (`browse --review-pack-hit-rect`). A band that contains one of those rectangles is captured even when it lies below the tile cap, labelled `hit band <n>` with `kind: "hit-band"`, so the reviewer opens the tile that shows the overflow or the runt instead of reading "no tile covers this rect". Coverage still reports the regular band prefix honestly; `hit_bands` on the context record counts the extras.
...
## `findings add`, `disposition`, `verdict`

The reviewer half of the loop. Machine findings in `evidence/critique.json` are claims; a reviewer confirms or dismisses each one and records defects the machine missed, and the verdict command turns both into a reviewed outcome that sits beside the machine outcome. `evidence/critique.json` is never edited; everything the reviewer writes goes into `findings.json`.

| Command | Description |
| --- | --- |
| `findings add <dir> --context <id> --tile <id> --severity <s> --category <c> --observation <text>` | Append a reviewer finding (`--tile` repeatable, at least one; `--recommendation`, `--id F001`, `--reviewer <name>` optional; ids auto-number). Validated against `findings.schema.json` before the write. |
...
| `verdict <dir> [--json]` | Compute the reviewed outcome and write `evidence/verdict.json` (`harnery-page-review-verdict/v1`), then refresh `review.md` with a "Reviewed outcome" section. A machine `high` counts unless its disposition is `artifact`, `not-a-defect`, or `duplicate-of-gate`; a reviewer finding of severity `critical` or `high` counts. Exit 2 when the reviewed outcome is `fail`, 4 when it is `incomplete` or `skipped`, 1 on validation errors. |
...
<pack>/
  ...
  findings.json              # reviewer-owned output (harnery-page-review-findings/v1): findings + dispositions
...
`review.md` carries the protocol. The short version: read the context table and coverage, glance at each context's contact sheet for orientation, open the inspection plan's primary tiles (page top, page bottom, every scoped tile, every hit band, every tile with a machine finding), treat machine findings as claims to confirm against the tile, cite deterministic gate failures rather than re-deriving them (their `gate_hits` name the tiles to look at), do not invent evidence for anything under "not checked", disposition every machine finding with `review-pack disposition`, add what the machine missed with `review-pack findings add`, and run `review-pack verdict`. Do it while the pack is alive: 90 minutes after the judge, the tiles are gone and only the report remains.
