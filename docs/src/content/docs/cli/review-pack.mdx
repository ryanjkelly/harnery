...
| Command | Description |
| --- | --- |
| `reviews add <dir> --reviewer <id> --model <name> --assigned <context>/<tile> --completed <context>/<tile>` | Record one review subagent's model, assignment, and completed native-pixel reads. `--model` accepts GPT-5.6 Luna, Composer 2.5, or Haiku 4.5. `--assigned` and `--completed` repeat; `--status complete|incomplete` defaults to `complete`. Repeating the reviewer id replaces its prior record. Only primary tiles may be assigned. |
