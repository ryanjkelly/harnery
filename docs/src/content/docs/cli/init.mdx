...
| `harn-end` | Finish a completed session with an authoritative V2 terminal event |
...
| `codex` | `.codex/hooks.json` | 9 native events; nested `{ hooks: [{ type, command }] }` entries; strict `description` + `hooks` top-level schema |
...
trusted while all nine Harnery hooks remain unable to run.
