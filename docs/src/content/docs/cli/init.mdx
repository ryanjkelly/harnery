...
3. **Injects the agent-facing guidance** so a fresh consumer's agent actually knows the coord surfaces exist (design: [ADR 0008](/decisions/0008-agent-facing-skills/)):
   ...
   - The generic **skills** `harn-decide` and `harn-council` written to `.claude/skills/` (claude-code only — Cursor and Codex read `AGENTS.md` natively). Suppress one with `skills.exclude` in `.harnery/config.jsonc` when you ship your own richer version.
