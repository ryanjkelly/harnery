...
```
harn init [--adapter <id> ...] [--instructions-only] [--dry-run] [--check] [--project-root <path>]
# <id> = claude-code | cursor | codex | opencode | all
```

`--adapter` is repeatable and also accepts comma-separated ids or `all`. With
no `--adapter` flag, `init` wires every adapter whose CLI is installed on this
machine, plus every adapter the project already has wired, so committed wiring
keeps refreshing on a box whose CLI is absent (CI, containers). A machine with
neither signal falls back to `claude-code`, the historical default.
...
3. **Wires the agent-hook entries** into the chosen adapter's settings file (see the adapter-specific counts below). For OpenCode, which has no hook file, it installs the Harnery plugin directory instead.
...
```bash
harn init                          # refresh every wired adapter (else claude-code)
harn init --adapter codex          # wire or refresh one adapter explicitly
harn init --adapter cursor --adapter opencode
harn init --adapter all            # wire every supported adapter
...
All four adapters are fully auto-wired. Pass `--adapter`:
...
| `opencode` | `.opencode/plugins/harnery/index.ts` | 12 events delivered by an in-process plugin, not a hooks map; see below |

Each writes only its own file, so several adapters can share one project: pass
`--adapter` more than once (or `all`), or just run `init` with no flag to wire
the harnesses installed on this machine and refresh everything already wired.
The canonical file path + event→subcommand map + entry shape for every adapter lives in [`src/core/hooks/adapter/events.ts`](https://github.com/ryanjkelly/harnery/blob/main/src/core/hooks/adapter/events.ts) (`ADAPTER_SPECS`).

### OpenCode is wired through a plugin

OpenCode V2 has no settings-file command hooks; the only interception point is
a plugin loaded into its shared background service. `harn init --adapter
opencode` therefore copies Harnery's OpenCode plugin to
`.opencode/plugins/harnery/index.ts` under a hash-stamped ownership header, and
writes `.opencode/plugins/harnery/harnery.json` naming the `agent-hook`
launcher. OpenCode auto-discovers that directory, so `opencode.json` is never
edited. The plugin bridges each lifecycle hook (`session.hook("prompt")`,
`tool.hook("execute.before"|"execute.after")`, `permission.hook("evaluate")`,
`session.hook("compaction")`, and the session bus) to `agent-hook <subcommand>
--adapter opencode`, and feeds context or a tool denial back through OpenCode's
own channels. OpenCode reads `AGENTS.md` and `.agents/skills` natively, so no
`CLAUDE.md` shim or skills mirror is written.

The plugin spawns the launcher from inside OpenCode's shared background
service, so a standalone project (Harnery installed from npm, launcher written
as the bare `agent-hook`) needs `agent-hook` on the PATH that service was
started with; restart the service after installing Harnery. A project that
contains Harnery gets a repo-relative launcher and needs nothing on PATH.

The plugin also prepends the project's own `bin/` directory and whichever Bun
install directory exists to every tool shell's PATH. An OpenCode server can
start from a GUI-launched environment whose PATH omits the project's tools,
which would otherwise turn an otherwise-valid `bin/<launcher>` call inside a
session into `command not found`.

`--check` treats a hand-edited plugin as drift and a foreign file at that path as
an error, and `init` refuses to overwrite a file it does not own. `harn deinit
--adapter opencode` removes the directory when Harnery owns it. The plugin
observes turn completion after the fact, so the end-of-turn ritual is recorded
but never re-prompted on OpenCode.
