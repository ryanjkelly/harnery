...
no `--adapter` flag, `init` refreshes every adapter the project already has
wired; a project with no wiring yet gets `claude-code`, the historical
default. Re-running `init` therefore keeps every harness in step instead of
silently refreshing only one.
...
`--adapter` more than once (or `all`), or re-run `init` with no flag after a
first explicit run to refresh everything already wired. The canonical file path + event→subcommand map + entry shape for every adapter lives in [`src/core/hooks/adapter/events.ts`](https://github.com/ryanjkelly/harnery/blob/main/src/core/hooks/adapter/events.ts) (`ADAPTER_SPECS`).
