...
a new name is first adopted. It refuses the transition when another fresh local
session or known remote session already has that name.
...
It prints the bare name on stdout (not a drawn box — a box would copy its borders
and labels too). `--json` returns `{ suggested_session_name, agent_name,
