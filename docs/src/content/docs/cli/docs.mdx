...
harn docs metadata validate [--repo <name>] [--require-v2]
...
`docs metadata validate` checks every file marked `schema: harnery-doc/v2`. It also
finds lifecycle documents and runbooks that have not moved to v2 yet:

```bash
harn docs metadata validate --repo .
harn docs metadata validate --repo . --require-v2
```

The report separates valid, invalid, legacy, and missing files. V2 validation
...
During the migration, legacy files are warnings. `--require-v2` promotes them
to errors so a repository can prove it is ready for cutover. That option is a
transition tool, not a permanent second mode. Once every managed repository is
on v2, the command will require v2 unconditionally and the option will be
removed.
