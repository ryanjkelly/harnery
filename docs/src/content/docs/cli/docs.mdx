...
- `docs lint --cached` validates the complete Git-index tree. During a
  path-scoped commit, Git supplies the proposed partial-commit index, so
  unrelated working-tree edits cannot change the verdict. Repository-wide
  entry and naming rules still run.
