...
`[path]` may name any of three things (default: `.qa-run`):
...
```bash
# What is the newest run under the default parent doing?
harn qa-status

# Reconnect to a detached run and exit with its verdict
harn qa-status .qa-run/run-6e1f0c3a-... --wait

# Poll a specific run from a script, machine-readable
harn qa-status .qa-run --json
