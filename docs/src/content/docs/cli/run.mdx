...
## Optional shadow pressure observation

Pass `--observe-pressure` to record local pressure before the first real child
dispatch. Harnery registers the run as a short-lived supervisor consumer,
demand-starts the optional supervisor when necessary, waits for one fresh
cycle, reads the same advice exposed by `harn diagnostics explain`, and then
unregisters the consumer.

This mode is observer-only. Normal, elevated, critical, and unknown advice all
leave dispatch unchanged. Startup, freshness, and read failures become explicit
unknown advice instead of failing or parking the run. The observer stays asleep
when every `agent()` call is served from cache or the workflow dispatches no
children.

The immutable run manifest freezes `{ schema_version: 1, mode: "shadow" }`.
Parked resume keeps the original choice and takes another sample only if it
reaches a new real dispatch. The transcript records `diagnostic.admission`; the
terminal `RunReport` and proof packet carry the same observation with
`action: "none"`.
...
- observer-only shadow admission advice when `--observe-pressure` was enabled;
