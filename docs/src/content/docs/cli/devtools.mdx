...
Reports, in one place, the on-disk state of the three AI coding agents Harnery
is built for: Claude Code (`~/.claude`), Codex (`~/.codex`), and Cursor
(`~/.cursor`). That covers logged-in status, plan / seat tier, auth expiry,
session counts, and, where the tool records them locally, rate-limit / quota
windows.

The default report comes from files on disk, with no network call. Auth tokens are
...
rollouts), with no endpoint. Unlike Claude Code and Cursor, there is nothing to
...
per in-window rollout rather than a full parse. That is cheap enough to run on every
render.
...
| `--usage` | Force a fresh, cache-bypassing recount of Claude Code's token total (the card already shows it, memoized). |
| `--window-days <n>` | Window for token totals: only count transcripts / rollouts modified within N days (default 7). |
