...
| `--no-pace` | Skip the human-pace gap between requests to the same site for this run. See [Human pace](/cli/browse/#human-pace). |
...
Requests are paced like every other Harnery page load: consecutive requests to the same site wait a random 3 to 9 seconds, shared across processes through `~/.cache/harnery/pace.json`, with local and private hosts exempt. `--no-pace` skips it for one run and `HARNERY_PACE=off` disables it machine-wide. The full set of controls is in [Human pace](/cli/browse/#human-pace).
