...
Save the important destinations with the artifact and print a compact Markdown
card for a chat handoff:
...
The first call validates every local destination, saves
`.harnery-delivery.json` in the workspace, and writes paste-ready Markdown to
standard output. The card contains a short linked list followed by a fenced
plain-text block. Both sections show the complete URL or filesystem path. The
artifact folder is always included automatically.

Run the command again without item options to reproduce the saved card:

```bash
harn artifacts delivery-card <artifact-id>
```

`--url` and `--path` are repeatable. URL entries must use HTTP or HTTPS. Path
