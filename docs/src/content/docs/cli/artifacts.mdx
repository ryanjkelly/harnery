...
Expired workspaces are also swept automatically: the first session start of
each day runs the same guarded deletion as `clean --yes`, so retention holds
...
`managed-expired` after the final recheck, and never touches unmanaged or
legacy directories. A repo with no sessions accumulates nothing and pays
nothing.
