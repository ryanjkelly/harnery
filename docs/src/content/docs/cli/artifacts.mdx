...
Artifact creation, managed `qa-run` results, and terminal `qa-status` reports
include `after_review` guidance with the appropriate discard command. This advice
never shortens retention by itself.
...
Session starts, `artifacts create`, and new managed QA workspaces offer a cleanup
opportunity at most once per hour. They run the same guarded deletion as
`clean --yes`. The sweep is throttled by a
...
contention does not consume the cleanup opportunity.
