...
Integration is deliberately separate from workflow success. The provider can
preview a fast-forward, but `prepareIntegration()` still requires satisfied
proof, accepted review, and policy authorization. `applyIntegration()` rereads
that authority and writes a durable receipt. Divergence, target movement,
dirty state, or a Git operation in progress blocks the apply. Harnery does not
resolve conflicts automatically.

Cleanup is also explicit. `cleanupWorkspace()` removes only resources covered
by the frozen binding and cleanup intent. Dirty or ambiguous work is preserved
for inspection.
