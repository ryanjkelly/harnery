...
If the policy returns `ASK`, preparation creates a durable approval and stops.
Resolve it with `workflow approvals`, then rerun the same prepare command. The
exact plan is reused and authorized. Apply requires `--yes` and rereads the
plan, proof, review, policy decision, approval, target identity, and attempt
chain before Git changes. Divergence, target movement, dirty state, or a Git
operation in progress blocks the apply. Harnery does not resolve conflicts.
