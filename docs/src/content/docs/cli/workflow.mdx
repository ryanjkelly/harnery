...
Rules return `allow`, `ask`, or `deny`. The standalone CLI fails closed on
`ask` because it has no implicit approval channel. An embedding host can supply
`EngineOpts.resolvePolicyAsk`; only an explicit valid approval proceeds.
Missing, invalid, throwing, or timed-out resolution denies. Use
