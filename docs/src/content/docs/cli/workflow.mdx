...
harn workflow workspace <run-id> [--json]
harn workflow workspaces [--json]
harn workflow integration prepare <run-id> --policy <file> [--reviewer <name>]
                                    [--reason <text>] [--target-root <dir>]
                                    [--accept-unknown <code>] [--approval-to <address>] [--json]
harn workflow integration apply <run-id> --yes [--json]
harn workflow cleanup <run-id> --yes [--json]
harn workflow reclaim <run-id> --yes [--discard] [--json]
...
types still require an embedding host provider. Ordinary CLI harness
...
policy-aware: `harn harness show cursor` still reports policy mapping as
...
```bash
harn workflow workspace wf-2026-07-24T02-10-00-000Z-a1b2c3
harn workflow workspace wf-2026-07-24T02-10-00-000Z-a1b2c3 --json
...
```bash
harn workflow integration prepare <run-id> \
...
```bash
harn workflow reclaim <run-id> --yes             # salvage the work, then release
harn workflow reclaim <run-id> --discard --yes   # throw the work away, then release
```
...
`harn workflow workspace <run-id>` lists the dirty paths, so the choice between
...
evidence appears as an error instead of a healthy badge. Journal-driven means a
