...
When `backup.schedule` is present, the session-start hook launches a snapshot in the background with the configured freshness duration and tags. The hook does not delay the agent session. It stays quiet on success. On failure it writes the hook's backup log under `.harnery/logs/` and prints one line on stderr.
