...
| `workflow:claude-code` / `workflow:codex` / `workflow:cursor` | optional | Spawn targets for [`harn workflow`](/cli/workflow/): is the harness CLI installed, and how will headless children bill (subscription login vs API key)? A missing CLI warns with the vendor's official install one-liner + login command; installed-but-unauthenticated warns with the login hint |
