...
The one exception is when a release **adds, renames, or retires a hook event**. Re-run [`harn init`](/cli/init) to migrate the Harnery-owned entries without touching third-party hooks. `harn doctor`'s `adapter hooks` check catches this, and a one-line nudge also appears at the start of your next session:
