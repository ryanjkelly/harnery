...
**Status:** deferred (2026-09-20; accepted 2026-08-15, never implemented) · **Scope:** product tier (`core/adapters`, `core/hooks`, `core/workflow`, `commands/init`, `commands/doctor`)

> **Deferred.** No Antigravity code exists in Harnery as of 2026-09-20; the four
> shipped adapters are Claude Code, Codex, Cursor, and OpenCode. The integration
> recipe below (registry profile, spawner with offline fixture, payload
> normalization, `init`/`doctor` wiring) was reused by
> [ADR 0189](./0189-opencode-adapter) and is the template to follow if this
> decision is picked up. Reactivate it with a dated status change and a plan;
> supersede it if Antigravity support is dropped for good.
...
Not implemented. The decision was accepted on 2026-08-15 but no slice shipped:
there is no `antigravity` adapter id, profile, spawner, hook translation, or
`init` branch in the codebase. The expected outcomes when the work is picked up
are unchanged:
...
- Lifecycle events, tool auditing, and turn-stop policies function in Antigravity environments.

ADR 0189 delivered the same recipe for OpenCode on 2026-09-20 and is the
