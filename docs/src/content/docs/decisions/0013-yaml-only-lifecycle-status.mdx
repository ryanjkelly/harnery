...
The legacy bold parser was retained temporarily inside
`docs frontmatter-migrate`. ADR 0083 superseded that transition: the command
and its parser were deleted after the host completed the `harnery-doc/v2`
cutover.

## Consequences

The initial host migration completed with zero remaining updates or errors from
the one-shot command. The later v2 cutover removed that command. Current
fixtures prove that bold-only and unversioned lifecycle files fail lint; hosts
must write `harnery-doc/v2` metadata directly.
