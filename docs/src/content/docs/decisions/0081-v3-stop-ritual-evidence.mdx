...
- `turn.completed.payload.ritual` records observation-wrapped status-box and
  session-name results. It contains booleans only. Cursor's Stop payload does
  not expose assistant text, but its earlier `afterAgentResponse` hook does.
  That hook reduces the completed reply to status-box booleans in owner-only
  producer state. Stop consumes the matching observation into
  `turn.completed` and clears the temporary state. The reply body never enters
  V3 intake, producer state, diagnostics, or the canonical ledger.
