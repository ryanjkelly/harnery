...
Once a host embeds Harnery this way, a second question follows immediately: *which of Harnery's internals may the host import?* The answer is the tier system. Hosts that need per-URL headers or cookies on `fetch` / `browse` wire `extraHeaders` and `extraCookies` on `HarneryProgramContext`; `--no-cookies` skips the cookie hook.
...
|---|---|---|---|
| **Product** | `harnery`, `harnery/commander`, `harnery/core/*` | The coordination layer: agents state, session events, hooks, journal, plus CLI composition. | The API to build against. Semver applies with full force here. |
| **Toolkit** | `harnery/lib/*` (`admission`, `http`, `cookies`, `format`, `readability`, `browser`, `agent-browser`, `pace`, `machine`, `tunnel/state`) | The supporting utilities Harnery's own CLI is built from, exposed so embedding hosts can lean on them instead of re-implementing. | Supported, but secondary. Can evolve faster than the product tier. Never the reason to adopt Harnery. |
