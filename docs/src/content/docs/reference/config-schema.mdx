...
Claude Code and Codex receive validated context directly. Cursor stages a
session-bound envelope and uses [`prompt-context consume`](/cli/prompt-context/)
because its current `beforeSubmitPrompt` hook cannot inject context.
