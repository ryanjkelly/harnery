...
1. `<project>/.harnery/config.jsonc`: the project settings file — **authoritative** for every field the config reader owns (`binName`, `hooksSetupHint`, `tools`, `workflow`, `skills`, `presence`, `files`)
2. CLI flags + environment variables: per-invocation overrides (see each field's env note)

> A user-global `~/.config/harnery/config.jsonc` layer is planned but **not yet consulted** by the primary reader (`src/core/config.ts`) — today only the project file is read. See [Where each section is read from](#where-each-section-is-read-from).
...
| Field                 | Type    | Default | Description |
| --------------------- | ------- | ------- | ----------- |
| `freshness_seconds`   | number  | `600`   | Heartbeat age above which an agent is considered stale (10 min) |
| `name_pool`           | string  | `default` | Name pool to draw from for new agents |

The freshness window is currently read from the env var `HARNERY_AGENT_COORD_FRESHNESS` (falling back to the `600` default), not yet from this `config.jsonc` key — see the note at the end of this page.
...
## Where each section is read from

Not every section above is read from `config.jsonc` yet. The fields Harnery's config reader (`src/core/config.ts`) consults from `config.jsonc` today are `project_name`, `binName`, `hooksSetupHint`, `tools`, `workflow`, `skills`, and `presence` (plus `files`, read by the web layer). The `coord`, `backup`, and `sync` sections are declared in the schema but are currently driven by **environment variables** (`HARNERY_AGENT_COORD_FRESHNESS`, `HARNERY_RESTIC_*`, `HARNERY_SYNC_*`) or a **separate file** (`sync` persists its remote/prefix in `~/.config/harnery/sync.json`), not by `config.jsonc`. Wiring those three to read from `config.jsonc` (or trimming them from the schema) is tracked separately; until then, set them via env/flags. The user-global `~/.config/harnery/config.jsonc` layer is likewise not yet consulted by the primary reader — project-level `.harnery/config.jsonc` is authoritative.
