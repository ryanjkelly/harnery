...
The config reader (`src/core/config.ts`) merges the two files and owns `binName`, `hooksSetupHint`, `agents`, `tools`, `workflow`, `skills`, `presence`, `coord`, `artifacts`, `backup`, and `sync`; the web layer reads `files`. Two values are project-only. The `binName` **pin** that `harn init` guards against re-stamping ignores the user file, and so does `agents.finalizationRoots`, because a global setting cannot grant filesystem authority to a project. A user-global `binName` is a display fallback, never a project pin.
...
| `finalization`        | object  | see below | Multi-signal V2 session-finalization grace periods |
...
These bounds control the canonical V2 reconciler. Archive and policy signals
...
| Field     | Type       | Default | Description |
| --------- | ---------- | ------- | ----------- |
| `exclude` | string[]   | `[]`    | Shipped Harnery skills this repo suppresses. The installed set is `harn-decide`, `harn-council`, and `harn-end`; the injected AGENTS.md block uses the matching CLI command instead of an excluded skill |
