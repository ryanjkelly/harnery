...
The config reader (`src/core/config.ts`) merges the two files and owns `binName`, `hooksSetupHint`, `hooks`, `agents`, `instructions`, `tools`, `workflow`, `skills`, `presence`, `coord`, `logs`, `storage`, `artifacts`, `backup`, and `sync`; the web layer reads `files`. Five values are project-only. The `binName` **pin** that `harn init` guards against re-stamping ignores the user file. So do `agents.finalizationRoots`, `instructions.promptReminder`, `hooks.promptContext`, and `storage.sharing`. A global setting cannot grant filesystem authority, execute a project extension, or carry one project's wording into another. A user-global `binName` is a display fallback, never a project pin.
...
| `storage`      | object  | see below        | Who can use the project's `.harnery/` state |
...
| Field     | Type   | Default   | Description |
| --------- | ------ | --------- | ----------- |
| `sharing` | string | `private` | `private` keeps every state directory `0700` and every state file `0600`, owned by whoever wrote it. `group` makes them `2770` (setgid) and `0660`, so several Unix users who share the project directory's group can coordinate in one project. It never opens state to other users |

Use `group` when agents running as different Unix users work in one checkout at the same time.
The project directory must belong to the shared group and be setgid, so new state inherits the
group, and each user must be a member. In group mode Harnery also clears the group bits from its
own umask, so a strict umask cannot turn shared state back into owner-only state.
Every integrity check that rejects loosely permissioned state follows the same setting: private
mode rejects any group or other access, and group mode rejects other access only.

Resolution: `HARNERY_STORAGE_SHARING` env → project `config.jsonc` → `private`. The
user-global file is ignored, because one person's setting must not loosen every project they use.
Personal state outside the project, such as browser cookies and sessions, stays owner-only either
way. See [ADR 0190](/decisions/0190-group-shared-project-state/).
...
| `storage` | `sharing` | `HARNERY_STORAGE_SHARING` | project file only |
