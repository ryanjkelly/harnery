...
The pid-map now anchors on the harness process, not on whatever shell happened
to run the hook.

Anchor selection recognised a harness by the name of its binary, and one Claude
...
Two ways in. A harness that exports its own pid is believed outright, which
