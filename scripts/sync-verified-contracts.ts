 ...
 *
 * Record an attestation first: `bin/harn harness attest --yes`.
 */
...
import {
  ...
  probeBinaryVersion,
  readAttestation,
} from "../src/core/harnesses/index.ts";
import { createBuiltinHarnessRegistry } from "../src/core/harnesses/registry.ts";
import type { HarnessProfile } from "../src/core/harnesses/types.ts";

const PROFILES_PATH = fileURLToPath(new URL("../src/core/harnesses/profiles.ts", import.meta.url));
...
function planFor(profile: HarnessProfile): Plan {
  ...
  if (!isAttestationCurrent(record, installed, profile)) {
    return {
      ...
      detail: record
        ? "an attestation exists but is stale; re-run `harn harness attest --yes`"
  ...
  const contradictions = Object.entries(record.observations)
    .filter(([dimension, observed]) => {
      const declared = profile.capabilities[dimension as keyof HarnessProfile["capabilities"]];
      return declared && declared.support !== observed;
    })
    .map(
      ([dimension, observed]) =>
        `${dimension} declared ${profile.capabilities[dimension as keyof HarnessProfile["capabilities"]].support}, observed ${observed}`,
    );
  ...
  if (profile.verified?.version === record.binary_version && profile.verified?.date === date) {
    return { harness: profile.id, action: "current", detail: record.binary_version };
  }
  return {
    harness: profile.id,
    action: "write",
 ...
 * profiles catalog is a plain object literal keyed by harness id, so the block
 * is located by its key rather than by line number. */
function rewrite(source: string, harness: string, date: string, version: string): string {
  // Object keys are quoted only when they are not valid identifiers, so
  // `"claude-code":` and `codex:` both appear in the same literal.
  const keyIndex = [`  "${harness}": {`, `  ${harness}: {`]
    .map((needle) => source.indexOf(needle))
    .find((index) => index !== -1);
  if (keyIndex === undefined) throw new Error(`could not locate profile block for ${harness}`);
  const verifiedIndex = source.indexOf("verified: {", keyIndex);
  if (verifiedIndex === -1) throw new Error(`could not locate verified block for ${harness}`);
  const end = source.indexOf("}", verifiedIndex);
  if (end === -1) throw new Error(`unterminated verified block for ${harness}`);
...
for (const plan of plans) {
  const label = plan.harness.padEnd(12);
  if (plan.action === "write") {
    console.log(`${label} ${check ? "STALE  " : "write  "} ${plan.detail}`);
    if (!check) source = rewrite(source, plan.harness, plan.date!, plan.version!);
    changed++;
...
if (!check && changed > 0) {
  ...
  console.log(`\nwrote ${changed} verified block(s) to src/core/harnesses/profiles.ts`);
} else if (!check) {
  console.log("\nno changes");
}
